...
v = x - A*x_prev - B*u;
    for i=1:N
        K1p = K1(1:ny, (i-1)*nu+1:i*nu);
        A_temp = At((i-1)*ny+1:i*ny, 1:nu);
        states_part = states_part + K1p*C*A_temp*x;
        controls_part = controls_part + K1p*C*Vp*(B*u + v);
        Vp = Vp+A_temp;
    end
du = Ke*Y_zad - states_part - controls_part;    % nie działa z N wiekszym niz 1 xD - sie poprawi
du2 = K1*(Y_zad - CtAt*x - CtV*B*u - CtV*v);    % na to samo wychodzi xD
